...
def print_analysis_results(results: Dict[str, object]) -> None:
    """Print a plain-text summary for the analysis."""
    print(f"\nSUMMARY FOR {results['platform'].upper()}")
    print("=" * 80)
    print(f"Total reviews: {results['total_reviews']}")
    print(f"Empty after preprocessing: {results['empty_after_preprocessing']}")
    percentage = 0.0
    ...
    if not details:
        print("No records became empty after stopword removal.")
        return

    for position, detail in enumerate(details, start=1):
        print("-" * 80)
        print(f"Empty review #{position} (index {detail['index']})")
        print(f"Original length: {detail['original_length']} characters")
        print(f"Original text: {detail['original_text']}")
        print(f"Cleaned text: {detail['cleaned_text']}")
        print(f"Tokens before stopwords ({detail['token_count_before_stopwords']}): {detail['tokens_before_stopwords']}")
        print(f"Tokens after stopwords ({detail['token_count_after_stopwords']}): {detail['tokens_after_stopwords']}")
        print(f"Became empty at: {detail['became_empty_at']}")
...
def main() -> None:
    """Run the analysis for both platforms."""
    print("=" * 80)
    print("EMPTY STRING ANALYSIS - PREPROCESSING PIPELINE INVESTIGATION")
    print("=" * 80)
    ...
    try:
        ...
        print(f"[OK] Loaded App Store data: {len(df_app)} reviews")
        df_play = load_dataset(play_path)
        print(f"[OK] Loaded Play Store data: {len(df_play)} reviews")
    ...
    print_analysis_results(app_results)
    print_analysis_results(play_results)

    print("=" * 80)
    print("CONCLUSION")
    print("=" * 80)
    print("Some reviews contain only stopwords after cleaning, leaving no tokens to model.")
    print("Filter these rows before training to avoid empty strings in the processed data.")
