...
APP_FILE = Path("lex_labeled_review_app.csv")
PLAY_FILE = Path("lex_labeled_review_play.csv")
...
def load_dataset() -> pd.DataFrame:
    ...
    if not APP_FILE.exists() or not PLAY_FILE.exists():
        raise FileNotFoundError("Source CSV files are missing in the workspace directory.")
...
def playstore_specific_insights(df: pd.DataFrame) -> None:
    play_df = df[df["Platform"] == "Play Store"].dropna(subset=["rating_score", "lexicon_to_rating"])
    if play_df.empty:
        return

    st.header("Play Store Evaluation Summary")

    mae = np.mean(np.abs(play_df["lexicon_to_rating"] - play_df["rating_score"]))
    rmse = np.sqrt(np.mean((play_df["lexicon_to_rating"] - play_df["rating_score"]) ** 2))
    pearson = play_df["rating_score"].corr(play_df["lexicon_to_rating"], method="pearson")
    spearman = play_df["rating_score"].corr(play_df["lexicon_to_rating"], method="spearman")

    metrics_df = pd.DataFrame(
        {
            "Metric": ["MAE", "RMSE", "Pearson r", "Spearman ρ"],
            "Value": [round(mae, 3), round(rmse, 3), round(pearson or 0.0, 3), round(spearman or 0.0, 3)],
        }
    )
    ...
    scatter_fig = px.scatter(
        ...
        title="Original vs Lexicon Rating (Play Store)",
    )
...
def model_performance_section(df: pd.DataFrame) -> None:
    ...
    with tfidf_tab:
        ...
        if tfidf_results:
            st.success(
                f"Best F1 (macro): {tfidf_results['best_score']:.3f} | Best Params: {tfidf_results['best_params']}"
            )
            ...
            st.dataframe(tfidf_results["ngram_summary"].assign(f1_macro=lambda x: x["f1_macro"].round(3)))
        else:
...
def main() -> None:
    ...
    playstore_specific_insights(filtered_df)
